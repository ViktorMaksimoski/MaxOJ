...
export const FocusProblem = ({ url, source, task, level }: FocusProblemProps) => {
    return (
    <button className="w-[100%] sm:w-[60%] mt-6 shadow-md hover:shadow-sm shadow-sky-300 text-left">
        <a href={url} target="__blank">
            <div className={`w-full bg-blue-500 rounded-t-md px-6 py-1`}>
                <h3 className="font-semibold text-xl sm:text-2xl text-white">{task}</h3>
                <p className="text-sm text-slate-100 font-semibold">{source} - {types[level].name}</p>
            </div>
